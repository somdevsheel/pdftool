    ...
    <ToolLayout toolTag="Delete Pages" title="Delete Pages" tagline="Click pages to mark for deletion, then download." icon="🗑️" accentColor={accent}>
      ...
      {phase === 'upload' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <UploadBox onFiles={handleDrop} maxFiles={1} label="Drop a PDF to remove pages from" />
      ...
      {phase === 'select' && rawFile && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ padding: '10px 14px', borderRadius: 6, fontSize: 12, background: 'rgba(193,126,232,0.1)', border: '1px solid rgba(193,126,232,0.25)', color: 'var(--text-soft)' }}>
            Click pages to mark for deletion — they will be removed from the output PDF.
          </div>
          {pageCount && <PageGrid file={rawFile} totalPages={pageCount} mode="delete" selectedPages={selected} pageOrder={pageOrder} onSelectedChange={setSelected} onOrderChange={setPageOrder} />}
      ...
      {phase === 'processing' && (
        <div style={{ padding: '64px 0', maxWidth: 440, margin: '0 auto', textAlign: 'center' }}>
          <ProgressBar job={job} />
          <p style={{ marginTop: 16, fontSize: 13, color: 'var(--text-muted)' }}>Removing pages with qpdf…</p>
        </div>
      )}

      {phase === 'done' && job && <ResultDownload jobId={job.id} filename="pages-deleted.pdf" onReset={handleReset} />}
