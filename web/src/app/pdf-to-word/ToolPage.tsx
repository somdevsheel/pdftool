...
export default function PdfToWordToolPage() {
  ...
  async function handleConvert() {
    ...
    try {
      const j = await createPdfToOfficeJob(file.id, 'docx');
      poll(j.id, (done) => {
    ...
    <ToolLayout title="PDF to Word" tagline="Convert PDF documents to editable Word files (.docx)." icon="📝" accentColor="#2B5EE8">
      ...
      {phase === 'configure' && file && rawFile && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 480 }}>
          ...
          <div style={{ padding: '12px 16px', borderRadius: 8, fontSize: 13, background: 'rgba(43,94,232,0.07)', border: '1px solid rgba(43,94,232,0.2)', color: 'var(--text-muted)' }}>
            ℹ️ Best results with text-based PDFs. Scanned PDFs may require OCR first.
          </div>
          {error && <p style={{ fontSize: 13, color: '#eb1000' }}>{error}</p>}
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={handleConvert} style={{ flex: 1, padding: '12px 0', borderRadius: 8, border: 'none', background: '#2B5EE8', color: '#fff', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
              Convert to Word →
            </button>
      ...
      {phase === 'processing' && (
        <div style={{ padding: '64px 0', maxWidth: 440, margin: '0 auto', textAlign: 'center' }}>
          <ProgressBar job={job} />
          <p style={{ marginTop: 16, fontSize: 13, color: 'var(--text-muted)' }}>Converting PDF to Word with LibreOffice…</p>
        </div>
      )}
      {phase === 'done' && job && <ResultDownload jobId={job.id} filename="converted.docx" onReset={handleReset} />}
      {phase === 'error' && (
