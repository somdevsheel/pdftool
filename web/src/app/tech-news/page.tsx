...
export default function TechNewsPage() {
  ...
  // featured loads immediately via dedicated request — never waits for scroll
  const { articles, featured, featuredLoading, loading, initialLoading, hasMore, loaderRef } = useInfiniteNews(activeTag);

  // Exclude featured articles from regular feed so nothing shows twice
  const featuredIds = new Set(featured.map(a => a._id));
  const regular = articles.filter(a => !featuredIds.has(a._id));
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg)' }}>
      ...
      <div className="max-w-7xl mx-auto w-full px-6 py-8 flex gap-8">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-8">
          ...
          {/* Featured — loads immediately, independent of scroll/pagination */}
