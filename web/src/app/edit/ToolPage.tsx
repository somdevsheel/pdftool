...
//     // Compute fit-to-width scale from container BEFORE first render
//     const firstPage = await pdf.getPage(1);
//     const naturalW = firstPage.getViewport({ scale: 1 }).width;
//     const containerW = containerRef.current ? containerRef.current.clientWidth - 56 : 800;
//     const fitScale = Math.floor((containerW / naturalW) * 100) / 100;
//     setZoom(fitScale);
//     await renderPage(1, fitScale);
//   }, [renderPage]);

//   useEffect(() => { if (rawFile && phase === 'edit') loadPdf(rawFile); }, [rawFile, phase]);
//   useEffect(() => { if (pdfRef.current && canvasSize.w > 0) renderPage(currentPage, zoom); }, [currentPage, zoom]);

//   async function handleFileSelect(f: File) {
//     const uploaded = await upload([f]);
//     if (uploaded?.[0]) { setFile(uploaded[0]); setRawFile(f); setPhase('edit'); }
//   }

//   // ── Drag & drop upload ────────────────────────────────────────────────────
//   const [draggingFile, setDraggingFile] = useState(false);
//   function handleFileDragOver(e: React.DragEvent) { e.preventDefault(); setDraggingFile(true); }
//   function handleFileDragLeave() { setDraggingFile(false); }
...
//     padding: '3px 10px', borderRadius: 4, border: `1px solid ${active ? accent : '#3a3a3a'}`,
//     background: active ? `rgba(194,65,12,0.15)` : 'transparent',
//     color: active ? '#fff' : '#bbb', fontWeight: active ? 700 : 400,
//     fontSize: 13, cursor: 'pointer', transition: 'all 0.12s',
//   });

//   const sideLabel: React.CSSProperties = {
//     fontSize: 10, fontWeight: 700, textTransform: 'uppercase',
//     letterSpacing: '0.07em', color: '#666', marginBottom: 2,
//   };

//   // ─────────────────── UPLOAD ────────────────────────────────────────────────
//   if (phase === 'upload') return (
//     <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#1a1a1a' }}>
//       <div
//         onDragOver={handleFileDragOver} onDragLeave={handleFileDragLeave} onDrop={handleFileDrop}
//         onClick={() => fileInputRef.current?.click()}
//         style={{
//           width: 420, padding: '52px 40px', borderRadius: 16, textAlign: 'center', cursor: 'pointer',
//           border: `2px dashed ${draggingFile ? accent : '#3a3a3a'}`,
//           background: draggingFile ? 'rgba(194,65,12,0.05)' : '#222',
//           transition: 'all 0.2s',
//         }}>
//         <div style={{ fontSize: 40, marginBottom: 16 }}>✏️</div>
//         <div style={{ fontSize: 18, fontWeight: 700, color: '#e8e8e8', marginBottom: 8 }}>Edit PDF</div>
//         <div style={{ fontSize: 13, color: '#888', marginBottom: 24 }}>Drop a PDF here or click to choose</div>
//         <div style={{ padding: '10px 28px', borderRadius: 8, background: accent, color: '#fff', fontWeight: 600, fontSize: 14, display: 'inline-block' }}>
...
//         <ProgressBar job={job} />
//         <p style={{ marginTop: 16, fontSize: 13, color: '#888' }}>Saving edits to PDF…</p>
//       </div>
//     </div>
//   );
...
//     <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#1a1a1a', flexDirection: 'column', gap: 16 }}>
//       <p style={{ color: '#eb1000', fontSize: 14 }}>{error}</p>
//       <button onClick={handleReset} style={{ fontSize: 13, color: '#888', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Try again</button>
//     </div>
//   );

//   // ─────────────────── EDITOR ────────────────────────────────────────────────
//   return (
//     <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#1a1a1a', color: '#e8e8e8', fontFamily: 'system-ui,sans-serif', overflow: 'hidden', position: 'fixed', inset: 0, zIndex: 999 }}>

//       {/* ── TOP BAR ── */}
//       <div style={{ height: 46, background: '#252525', borderBottom: '1px solid #333', display: 'flex', alignItems: 'center', gap: 8, padding: '0 14px', flexShrink: 0, zIndex: 30 }}>
//         {/* Close */}
//         <button onClick={handleReset} title="Close editor"
//           style={{ width: 28, height: 28, borderRadius: 6, border: 'none', background: '#3a3a3a', color: '#aaa', fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
//           ×
//         </button>
//         <div style={{ width: 1, height: 22, background: '#3a3a3a', margin: '0 4px' }} />
//         <span style={{ fontSize: 13, fontWeight: 600, color: '#ccc', marginRight: 8 }}>Edit PDF</span>
...
//         {/* Font controls — shown when text tool or item selected */}
...
//           style={{ background: '#333', border: '1px solid #444', color: '#e8e8e8', borderRadius: 4, padding: '3px 6px', fontSize: 12, cursor: 'pointer', width: 130 }}>
...
//           style={{ width: 48, background: '#333', border: '1px solid #444', color: '#e8e8e8', borderRadius: 4, padding: '3px 6px', fontSize: 12, textAlign: 'center' }} />
...
//         {/* Color swatch */}
//         <div style={{ width: 24, height: 24, borderRadius: 4, background: selected ? selected.color : color, border: '1px solid #555', overflow: 'hidden', position: 'relative', cursor: 'pointer', flexShrink: 0 }}>
...
//             style={{ position: 'absolute', inset: -2, width: '140%', height: '140%', opacity: 0, cursor: 'pointer' }} />
//         </div>
...
//           style={{ ...topBtn(false), padding: '3px 8px' }}>−</button>
...
//           const fitScale = Math.floor(((containerRef.current.clientWidth - 56) / naturalW) * 100) / 100;
//           setZoom(fitScale);
//         }} style={{ ...topBtn(false), padding: '3px 8px', fontSize: 11 }}>Fit</button>

//         <div style={{ width: 1, height: 22, background: '#3a3a3a', margin: '0 4px' }} />

//         {/* Page nav */}
//         <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage <= 1}
//           style={{ ...topBtn(false), opacity: currentPage <= 1 ? 0.4 : 1 }}>‹</button>
//         <span style={{ fontSize: 12, color: '#999', minWidth: 64, textAlign: 'center' }}>
//           {pdfRendering ? '…' : `${currentPage} / ${numPages || '?'}`}
//         </span>
//         <button onClick={() => setCurrentPage(p => numPages ? Math.min(numPages, p + 1) : p + 1)}
//           disabled={numPages > 0 && currentPage >= numPages}
//           style={{ ...topBtn(false), opacity: (numPages > 0 && currentPage >= numPages) ? 0.4 : 1 }}>›</button>

//         <div style={{ width: 1, height: 22, background: '#3a3a3a', margin: '0 4px' }} />

//         {/* Save */}
//         <button onClick={handleSave} disabled={validItems.length === 0}
//           style={{ padding: '5px 20px', borderRadius: 6, border: 'none', background: validItems.length > 0 ? accent : '#3a3a3a', color: validItems.length > 0 ? '#fff' : '#666', fontWeight: 700, fontSize: 13, cursor: validItems.length > 0 ? 'pointer' : 'not-allowed', transition: 'background 0.15s' }}>
...
//                 color: sideTab === tab ? '#fff' : '#666', fontSize: 12, fontWeight: 600,
//                 cursor: 'pointer', borderBottom: `2px solid ${sideTab === tab ? accent : 'transparent'}`,
...
//                 {/* Tool hint */}
//                 <div style={{ padding: '10px 12px', borderRadius: 8, background: activeTool === 'text' ? 'rgba(194,65,12,0.1)' : '#2a2a2a', border: `1px solid ${activeTool === 'text' ? 'rgba(194,65,12,0.3)' : '#333'}`, fontSize: 12, color: '#999', lineHeight: 1.5 }}>
//                   {activeTool === 'text'
//                     ? <><span style={{ color: accent, fontWeight: 600 }}>Text tool active</span><br />Click anywhere on the PDF to add text</>
//                     : <><span style={{ color: '#bbb', fontWeight: 600 }}>Select tool active</span><br />Click a text item to select and drag it</>
//                   }
//                 </div>
...
//                         style={{ padding: '4px 0', borderRadius: 5, border: `1px solid ${(selected ? selected.fontSize : fontSize) === s ? accent : '#3a3a3a'}`, background: (selected ? selected.fontSize : fontSize) === s ? 'rgba(194,65,12,0.15)' : '#2a2a2a', color: (selected ? selected.fontSize : fontSize) === s ? accent : '#999', fontSize: 11, cursor: 'pointer' }}>
//                         {s}
//                       </button>
//                     ))}
//                   </div>
...
//                     ))}
//                     <div style={{ width: 24, height: 24, borderRadius: '50%', border: '1px solid #444', overflow: 'hidden', position: 'relative', flexShrink: 0 }}>
//                       <input type="color" value={selected ? selected.color : color}
//                         onChange={e => { if (selected) updateSelected({ color: e.target.value }); else setColor(e.target.value); }}
//                         style={{ position: 'absolute', inset: -3, width: '150%', height: '150%', cursor: 'pointer', border: 'none', opacity: 1 }} />
//                     </div>
//                   </div>
//                 </div>
...
//                   <div>
//                     <div style={sideLabel}>On this page ({pageItems.filter(it => it.text.trim()).length})</div>
...
//                           <span style={{ fontSize: 10, color: '#555' }}>{it.fontSize}pt</span>
...
//                     style={{ padding: '8px 0', borderRadius: 6, border: '1px solid #e33', background: 'rgba(235,16,0,0.08)', color: '#e33', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
//                     Delete selected
//                   </button>
//                 )}

//                 {/* How-to */}
//                 <div style={{ padding: '10px 12px', borderRadius: 8, background: '#2a2a2a', border: '1px solid #333', fontSize: 11, color: '#666', lineHeight: 1.7 }}>
//                   <strong style={{ color: '#888' }}>How to use</strong><br />
//                   1. Select <em>Add Text</em> tool<br />
//                   2. Click on the PDF to place<br />
//                   3. Type your text, press Enter<br />
//                   4. Drag to reposition<br />
//                   5. Double-click to re-edit<br />
...
//                 <div style={{ fontSize: 12, color: '#666' }}>{numPages} page{numPages !== 1 ? 's' : ''}</div>
//                 {Array.from({ length: numPages }, (_, i) => i + 1).map(p => (
//                   <button key={p} onClick={() => setCurrentPage(p)}
//                     style={{ padding: '8px 12px', borderRadius: 6, border: `1px solid ${currentPage === p ? accent : '#333'}`, background: currentPage === p ? 'rgba(194,65,12,0.12)' : '#2a2a2a', color: currentPage === p ? '#fff' : '#999', fontSize: 12, cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8 }}>
//                     <span style={{ width: 20, height: 20, borderRadius: 4, background: '#333', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, flexShrink: 0 }}>{p}</span>
//                     Page {p}
...
//           style={{ flex: 1, overflow: 'auto', background: '#404040', display: 'flex', justifyContent: 'center', padding: '28px 28px 60px', cursor: activeTool === 'text' ? 'text' : 'default' }}
...
//                         whiteSpace: 'nowrap',
//                         lineHeight: 1.3,
//                         cursor: activeTool === 'select' || isSelected ? 'move' : 'text',
//                         padding: '0 3px',
//                         background: isSelected ? 'rgba(194,65,12,0.1)' : 'transparent',
//                         outline: isSelected ? `2px dashed ${accent}` : item.text ? '2px dashed transparent' : `2px dashed rgba(194,65,12,0.4)`,
//                         outlineOffset: 2,
//                         borderRadius: 2,
//                         minWidth: item.text ? undefined : 60,
//                       }}>
//                       {item.text || <span style={{ color: 'rgba(194,65,12,0.6)', fontStyle: 'italic', fontSize: `${Math.min(item.fontSize * zoom, 13)}px` }}>Click to type…</span>}
//                     </span>
//                   )}
...
//                       style={{ position: 'absolute', top: -8, right: -8, width: 18, height: 18, borderRadius: '50%', background: '#eb1000', border: '2px solid #fff', color: '#fff', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1, zIndex: 30, boxShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>
//                       ×
//                     </button>
//                   )}
//                 </div>
...
//       {/* ── STATUS BAR ── */}
//       <div style={{ height: 26, background: '#1e1e1e', borderTop: '1px solid #2e2e2e', display: 'flex', alignItems: 'center', padding: '0 14px', gap: 16, flexShrink: 0 }}>
...
//   if (phase === 'upload') return (
//     <ToolLayout title="Edit PDF" tagline="Add text to your PDF — click to place, drag to reposition." icon="✏️" accentColor={accent}>
//       <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
//         <UploadBox
//           onFiles={handleDrop}
//           maxFiles={1}
