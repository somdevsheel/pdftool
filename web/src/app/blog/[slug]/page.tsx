...
export default function BlogPostPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    fetch(`${API_BASE}/api/blog/slug/${slug}`)
      .then(r => r.json())
      .then(data => { if (data.post) setPost(data.post); else setNotFound(true); })
      .catch(() => setNotFound(true))
      .finally(() => setLoading(false));
  }, [slug]);

  // Show subscribe popup at 50% scroll
  useEffect(() => {
    const handleScroll = () => {
      ...
      <article className="max-w-4xl mx-auto px-6 py-10" ref={articleRef}>
        ...
        <div className="flex items-center gap-3 mb-4">
          <span className="text-xs font-semibold px-3 py-1 rounded-full"
            style={{ background: `${post.tagColor}22`, color: post.tagColor }}>{post.tag}</span>
        ...
        <h1 className="font-bold text-3xl leading-tight mb-4" style={{ color: 'var(--text)' }}>{post.title}</h1>
        <p className="text-lg leading-relaxed mb-6" style={{ color: 'var(--text-muted)' }}>{post.summary}</p>

        {/* Author */}
        <div className="flex items-center gap-3 mb-8 pb-6" style={{ borderBottom: '1px solid var(--border)' }}>
          <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold text-white" style={{ background: 'var(--accent)' }}>
        ...
        <div>
          <ReactMarkdown components={{
            ...
            p: ({children}) => <p style={{ color: 'var(--text-muted)', lineHeight: '1.8', marginBottom: '1.25rem' }}>{children}</p>,
            ...
            blockquote: ({children}) => <blockquote style={{ borderLeft: '4px solid var(--accent)', paddingLeft: '1rem', marginBottom: '1.25rem', opacity: 0.85 }}>{children}</blockquote>,
            code: ({children, className}) => className?.includes('language-')
              ? <pre style={{ background: 'var(--surface)', borderRadius: '0.5rem', padding: '1rem', overflowX: 'auto', marginBottom: '1.25rem' }}><code style={{ color: '#eb1000', fontSize: '0.875rem', fontFamily: 'monospace' }}>{children}</code></pre>
        ...
        <div className="mt-12 pt-8 flex items-center justify-between" style={{ borderTop: '1px solid var(--border)' }}>
          <Link href="/blog" className="text-sm px-4 py-2 rounded-lg" style={{ color: 'var(--text-muted)', border: '1px solid var(--border)' }}>
            ← All Posts
          </Link>
