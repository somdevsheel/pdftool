...
//     try {
//       const j = await createPdfToImgJob(file.id, format, dpi);
//       poll(j.id, (done) => {
...
//     setPhase('upload'); setError(null); resetJob();
//   }
...
//   return (
//     <ToolLayout title="PDF to JPG" tagline="Convert every PDF page to a JPG or PNG image. Downloaded as a ZIP." icon="🖼️" accentColor={accent}>
...
//                     ...btnBase,
//                     border: `2px solid ${format === f ? accent : 'var(--border)'}`,
//                     background: format === f ? 'rgba(193,126,232,0.1)' : 'var(--surface-2)',
//                     color: format === f ? accent : 'var(--text-muted)',
...
//                     ...btnBase,
//                     border: `2px solid ${dpi === d ? accent : 'var(--border)'}`,
//                     background: dpi === d ? 'rgba(193,126,232,0.1)' : 'var(--surface-2)',
//                     color: dpi === d ? accent : 'var(--text-muted)',
//                   }}>{d} DPI{d === 150 ? ' · Fast' : d === 300 ? ' · HQ' : ''}</button>
//                 ))}
//               </div>
//             </div>
...
//               📦 Each page becomes an image — all bundled into a ZIP file.
//             </div>
//           </div>

//           {error && <p style={{ fontSize: 13, color: '#eb1000', background: 'rgba(235,16,0,0.08)', padding: '10px 14px', borderRadius: 6 }}>{error}</p>}
...
//           <ProgressBar job={job} />
//           <p style={{ marginTop: 16, fontSize: 13, color: 'var(--text-muted)' }}>Converting PDF pages to {format.toUpperCase()} at {dpi} DPI…</p>
//         </div>
//       )}

//       {phase === 'done' && job && (
//         <ResultDownload jobId={job.id} filename="pdf-images.zip" isZip={true} onReset={handleReset} />
//       )}
