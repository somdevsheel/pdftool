...
export default function PdfToExcelToolPage() {
  ...
  async function handleConvert() {
    ...
    try {
      const j = await createPdfToOfficeJob(file.id, 'xlsx');
      poll(j.id, (done) => {
    ...
    <ToolLayout title="PDF to Excel" tagline="Convert PDF tables to editable Excel spreadsheets (.xlsx)." icon="📈" accentColor="#1E7E34">
      ...
      {phase === 'upload' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <UploadBox onFiles={handleDrop} maxFiles={1} label="Drop a PDF to convert to Excel" accept=".pdf,application/pdf" />
      ...
      {phase === 'configure' && file && rawFile && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 480 }}>
          ...
          <div style={{ padding: '12px 16px', borderRadius: 8, fontSize: 13, background: 'rgba(30,126,52,0.07)', border: '1px solid rgba(30,126,52,0.25)', color: 'var(--text-muted)' }}>
            ℹ️ Best results with PDFs that contain clear table structures. Scanned PDFs may need OCR first.
          </div>
          {error && <p style={{ fontSize: 13, color: '#eb1000' }}>{error}</p>}
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={handleConvert} style={{ flex: 1, padding: '12px 0', borderRadius: 8, border: 'none', background: '#1E7E34', color: '#fff', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
              Convert to Excel →
            </button>
      ...
      {phase === 'processing' && (
        <div style={{ padding: '64px 0', maxWidth: 440, margin: '0 auto', textAlign: 'center' }}>
          <ProgressBar job={job} />
          <p style={{ marginTop: 16, fontSize: 13, color: 'var(--text-muted)' }}>Converting PDF to Excel with LibreOffice…</p>
        </div>
      )}
      {phase === 'done' && job && <ResultDownload jobId={job.id} filename="converted.xlsx" onReset={handleReset} />}
      {phase === 'error' && (
