...
export default function BlogPostForm({ initialData, postId }: BlogPostFormProps) {
  ...
  const labelStyle = { color: '#888', fontSize: '13px', marginBottom: '6px', display: 'block' as const };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 flex flex-col gap-5">
        ...
        <div style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: '12px', padding: '20px' }}>
          ...
          <p style={{ color: '#444', fontSize: '11px', marginTop: '8px' }}>
            ## Heading · **bold** · *italic* · - list · [link](url) · `code`
          </p>
        </div>
        ...
        <div style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: '12px', padding: '20px' }}>
          <div className="flex items-center justify-between mb-3">
            <label style={{ ...labelStyle, marginBottom: 0 }}>Featured Image</label>
            <span style={{ fontSize: '11px', color: '#555', background: '#1a1a1a', padding: '2px 8px', borderRadius: '20px' }}>
              Optional
            </span>
          </div>
          <ImageUploader
          ...
          {!form.imageUrl && (
            <p style={{ color: '#444', fontSize: '11px', marginTop: '8px' }}>
              No image? A default placeholder will be used.
            </p>
          )}
        </div>
        ...
        {error && (
          <p className="text-sm px-4 py-3 rounded-lg"
            style={{ background: '#eb100011', color: '#eb1000', border: '1px solid #eb100033' }}>
            {error}
          </p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div style={{ background: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: '12px', padding: '16px' }}>
          <h3 className="text-xs font-semibold mb-3 uppercase tracking-wider" style={{ color: '#555' }}>Markdown Guide</h3>
          {[
            ...
            <div key={syntax} className="flex items-center justify-between py-1.5" style={{ borderBottom: '1px solid #111' }}>
              <code style={{ color: '#eb1000', fontSize: '11px', fontFamily: 'monospace' }}>{syntax}</code>
              <span style={{ color: '#555', fontSize: '11px' }}>{desc}</span>
            </div>
          ))}
        </div>
      </div>
